...
def _get_checkpoint_manager(
  ...
  return orbax_checkpoint.CheckpointManager(
      ...
      options=orbax_checkpoint.CheckpointManagerOptions(
          max_to_keep=30, create=True, keep_period=keep_period
      ),
  )
...
def train_and_evaluate(
  ...
  # Chepoints lists
  eval_log_steps = [int(config.eval_every_log_steps**i) for i in range(0, int(np.log(num_train_steps) / np.log(config.eval_every_log_steps)) + 1)]
  checkpoint_log_steps = [int(config.checkpoint_every_log_steps**i) for i in range(0, int(np.log(num_train_steps) / np.log(config.checkpoint_every_log_steps)) + 1)]
